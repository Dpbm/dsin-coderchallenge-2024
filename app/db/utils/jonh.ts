...
export function classify(spaceship:Spaceship|SpaceshipRow, model:tf.LayersModel) : string{
    ...
    const input = [value_enc, power_enc, damage_enc, size_enc, danger_enc, gas_enc];
    const prediction = model.predict(tf.tensor(input).reshape([-1,6]))
...
function decodePrediction(prediction:number[]):string{
    ...
    const labels = ["Sucata Espacial", "Joia Tecnológica", "Arsenal Alienígena", "Ameaça em Potencial", "Fonte de Energia Alternativa"];
